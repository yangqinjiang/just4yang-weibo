...
import cn.yang.sina.weibo.R;
	 ...
	 *
	 */
	public class WeiboFavoritesViewHolder{
		...
		public WeiboFavoritesViewHolder(Context context){
		 ...
		 * @param status 微博内容
		 */
		public void bindStatusData(Favorite fav,int position){
			Log.i(TAG, "bindStatusData...");
			if(!fav.isNull()){
				User user =fav.getStatus().getUser();
				if(!user.isNull()){
			...
			//创建时间
			this.getTvCreatedTime().setText(fav.getStatus().getCreated_at()+"");
			//新浪认证
			if(user.isVerified())
			{
			...
			//微博内容
			String content =fav.getStatus().getText();
			...
			//判断原微博是否有缩略图
			if(!StringUtil.isEmpty(fav.getStatus().getThumbnail_pic())){
				//if(Task.DEBUG)Log.i(TAG, "第  "+position+" 项"+"有图片");
					SimpleImageLoader.showImg(fav.getStatus().getThumbnail_pic().toString(),this.getIvThumbnailPic());
			...
			//判断是否有转发的weibo
			Status reStatus =fav.getStatus().getRetweeted_status();
